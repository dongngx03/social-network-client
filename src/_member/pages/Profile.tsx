...
const Profile = () => {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            try {
                const { data } = await instance.get(`/v1/api/post/get-post-one-user/${user?.user?.id}`)
                return data
            } catch (error) {
                console.log(error);
            }
        ...
        <div className='w-full h-full overflow-auto'>
            <div className="profile">
                ...
                <div className="w-full h-auto flex flex-col px-6 pt-10">
                        ...
                        <div className="w-full flex">
                            ...
                            <div className="w-2/3 flex flex-col gap-5 justify-center">
                                <div className=" flex justify-start gap-5 items-center">
                                    <span className="text-[20px] font-normal font-sans">{user?.user?.nickname}</span>
                                    <Link to={"/profile/edit"}><button className="bg-gray-200 px-3 py-[6px] text-sm rounded-md font-semibold font-sans hover:bg-[#DBDBDB]">Chỉnh sửa trang cá nhân</button></Link>
                                ...
                                <div>
                                    <span className="font-sans font-medium text-md">
                                        {`${user?.user?.fistname} ${user?.user?.lastname}`}
                                    </span>
                                </div>
                                <div>
                    ...
                    <div className=" overflow-auto relative w-full h-1/2 py-10 px-10 flex justify-start gap-10">
                ...
                {/* post */}
                <div className="w-full h-auto px-16 pb-10 flex flex-col ">
